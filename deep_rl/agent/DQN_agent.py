...
class DQNAgent(BaseAgent):
    ...
    # new function to decompose the reward function to multi-dimension
    def decompose(self, primitive_reward): # [bs]
        # reward_list = [0.0 for _ in range(self.config.reward_dim)]
        reward_list = torch.zeros(primitive_reward.shape[0], self.config.reward_dim).to(self.config.DEVICE)
        for i, p_reward in enumerate(primitive_reward):
            remain_reward = p_reward
            for index, (reward, cond) in enumerate(self.config.rewards.items()): # loop: reward, condition
                is_valid = remain_reward >= reward and eval(cond.format(remain_reward - reward))
                if is_valid:
                    remain_reward -= reward
                    reward_list[i, index] += reward
        return reward_list
